...
const manage=page=>page.getByRole('button',{name:'설정',exact:true}).click();
test('all content, dated records, Finder navigation, search and window controls',async({page})=>{
 ...
 await page.getByRole('button',{name:'만든 프로그램 폴더',exact:true}).click();await expect(page.locator('.folder-app')).toHaveCount(7);
 await page.locator('.folder-app').filter({hasText:'ExamPool'}).click();await expect(page.getByRole('button',{name:/설명 보기/})).toBeVisible();
 await page.getByRole('button',{name:'이전 화면',exact:true}).click();await expect(page.locator('.folder-app')).toHaveCount(7);
 await page.getByRole('button',{name:'창 최소화'}).click();await expect(page.locator('#workspace-window')).not.toBeVisible();await page.locator('#restore').click();await expect(page.locator('.folder-app')).toHaveCount(7);await close(page);
 ...
 await page.getByRole('button',{name:'만든 프로그램 폴더',exact:true}).click();await page.getByRole('button',{name:'프로젝트',exact:true}).click();await expect(page.locator('.step')).toHaveCount(3);await close(page);
 await page.getByRole('button',{name:'소개',exact:true}).click();await expect(page.locator('.channel')).toHaveCount(6);expect(errors).toEqual([]);
});
test('draft note and icon shortcut persist, public view stays unchanged, publish is owner-only',async({page})=>{
 ...
 await page.reload();await page.getByRole('button',{name:'설정',exact:true}).click();await expect(page.getByLabel('방문자에게 남기는 메모')).toHaveValue('테스트 방문자 메모');
 ...
 await manage(page);await page.getByRole('button',{name:'Google 관리자 로그인'}).click();await expect(page.getByRole('button',{name:'공개 게시',exact:true})).toBeEnabled();page.once('dialog',d=>d.accept());await page.getByRole('button',{name:'공개 게시',exact:true}).click();await expect(page.getByRole('heading',{name:'게으른 교사의 작업실',exact:true})).toBeVisible();expect(writes).toHaveLength(1);expect(writes[0].fields.note.stringValue).toBe('테스트 방문자 메모');
});
test('record and categorized skill CRUD plus backup validation and conflict recovery',async({page})=>{
 ...
 await page.getByRole('button',{name:'설정',exact:true}).click();await page.getByRole('button',{name:'Google 관리자 로그인'}).click();page.once('dialog',d=>d.accept());await page.getByRole('button',{name:'공개 게시',exact:true}).click();await expect(page.locator('#admin-status')).toContainText('다른 곳에서');
...
for(const width of [375,768,1280])test(`responsive surfaces ${width}`,async({page})=>{
 ...
 for(const name of ['만든 프로그램 폴더','추천 도구함','연수 자료','공유 자료','소개']){await page.getByRole('button',{name,exact:true}).first().click();await page.screenshot({animations:'disabled',path:`.omo/evidence/studio/${name}-${width}.png`});expect(await page.locator('#window-body').evaluate(el=>el.scrollWidth<=el.clientWidth+1)).toBe(true);await close(page);}
...
test('reduced motion, physics pause and keyboard Escape',async({page})=>{await page.emulateMedia({reducedMotion:'reduce'});await mock(page);await page.getByRole('button',{name:'만든 프로그램 폴더',exact:true}).click();await page.getByRole('button',{name:'놀이터',exact:true}).click();await expect(page.getByRole('button',{name:'재생',exact:true})).toBeVisible();await page.getByRole('slider').fill('200');await expect(page.locator('#period')).toContainText('2.84');await page.keyboard.press('Escape');await expect(page.locator('#workspace-window')).not.toBeVisible();});
...
test('screen entry, distinct shortcut control, exit and in-desktop app browser',async({page})=>{
 await mock(page);await page.getByRole('button',{name:'만든 프로그램 폴더',exact:true}).click();await page.getByRole('button',{name:'5E',exact:true}).click();await expect(page.getByRole('button',{name:/설명 보기/})).toBeVisible();await page.getByRole('button',{name:/바로 실행/}).click();await expect(page.locator('#browser-frame')).toHaveAttribute('src','https://seungyeon980808-pixel.github.io/5E/');const rect=await page.locator('#workspace-window').boundingBox();expect(rect.width).toBeLessThan(1280);expect(rect.height).toBeLessThan(900);await page.locator('#titlebar [data-exit]').click();await expect(page.locator('#entrance')).toHaveAttribute('data-phase','closed');await expect(page.locator('#workspace-window')).not.toBeVisible();await page.locator('#enter').click();await expect(page.locator('#entrance')).toBeHidden();await manage(page);await close(page);await expect(page.locator('.shortcut-create')).toContainText('새 바로가기');await page.locator('.shortcut-create').click();await expect(page.getByLabel('연결 주소')).toBeVisible();
});
...
test('short resource and note windows fit their content and still maximize',async({page})=>{
 ...
 await page.getByRole('button',{name:'메모 펼쳐보기 ↗',exact:true}).click();await page.waitForTimeout(260);expect((await win.boundingBox()).height).toBeLessThan(compact.height);
});
