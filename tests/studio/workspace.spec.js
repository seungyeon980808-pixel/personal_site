...
async function mock(page,{auth='owner',conflict=false,remote=null}={}){
 ...
 await page.goto('/?entrance=photo');await page.getByRole('button',{name:/(노트북 열고|휴대폰 들고) 작업실 들어가기/,exact:true}).click();await expect(page.locator('#entrance')).toBeHidden();return writes;
}
...
test('all content, dated records, Finder navigation, search and window controls',async({page})=>{
 ...
 await page.getByRole('button',{name:'인사드립니다',exact:true}).focus();await page.keyboard.press('Enter');await expect(page.getByRole('heading',{name:'인사드립니다',exact:true})).toBeVisible();await expect(page.locator('.about-intro-copy')).toContainText('저는 게으른 교사입니다.');await expect(page.locator('.about-page')).toContainText('교사가 한가해야');expect(errors).toEqual([]);
});
test('draft note and icon shortcut persist, public view stays unchanged, publish is owner-only',async({page})=>{
 ...
 await page.getByRole('button',{name:'공개 화면 보기'}).click();await expect(page.locator('#playful-desktop')).toBeVisible();await expect(page.locator('#icons a')).toHaveCount(0);await expect(page.locator('#note-text')).not.toHaveText('테스트 방문자 메모');
...
for(const width of [375,768,1280])test(`responsive surfaces ${width}`,async({page})=>{
 await page.setViewportSize({width,height:900});await page.emulateMedia({reducedMotion:'reduce'});await mock(page);
...
test('owner memo checklist and position persist separately from public content, with optional archive and custom project folders',async({page})=>{
 ...
 await expect(page.locator('#playful-desktop')).toBeHidden();await expect(page.locator('#icons').getByRole('button',{name:'인사드립니다',exact:true})).toBeVisible();await expect(page.locator('#icons').getByRole('button',{name:'연수 자료',exact:true})).toBeVisible();
