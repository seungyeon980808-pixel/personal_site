...
test('six notch friends load and disappear when clicked',async({page})=>{
 await page.emulateMedia({reducedMotion:'reduce'});
 await page.addInitScript(()=>{localStorage.setItem('studio-welcome-v1','done');sessionStorage.setItem('studio-entered','1');});await page.goto('/?entrance=photo');
 const notch=page.locator('.desktop-notch');await expect(notch).toBeVisible();await expect(notch.locator('button')).toHaveCount(6);
