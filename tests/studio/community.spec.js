...
async function prepare(page,{failure=false,owner=false}={}){
 ...
 await page.route('https://firestore.googleapis.com/**',async route=>{
  ...
  if(url.includes(':commit')){const body=req.postDataJSON();expect(req.headers().authorization).toBe('Bearer guest-token');writes.push(body);if(failure)return route.fulfill({status:403,json:{error:{code:403}}});const write=body.writes[0];entries.push({...write.update,fields:{...write.update.fields,createdAt:{timestampValue:'2026-09-13T00:00:00Z'}}});return route.fulfill({json:{writeResults:[{}]}});}
 ...
 await page.route('https://www.gstatic.com/firebasejs/**',route=>route.fulfill({contentType:'application/javascript',body:route.request().url().includes('firebase-app')?'export const initializeApp=(config,name)=>({name});export const getApps=()=>[];':`const user={email:'seungyeon980808@gmail.com',emailVerified:true,getIdToken:async()=> 'test-token'};const guest={uid:'test-guest',getIdToken:async()=> 'guest-token'};export const getAuth=app=>({currentUser:app?.name==='comment-guest'?null:${owner?'user':'null'},authStateReady:async()=>{}});export const signInAnonymously=async()=>({user:guest});export const onAuthStateChanged=(a,cb)=>cb(a.currentUser);export class GoogleAuthProvider{};export const signInWithPopup=async()=>({user});export const signOut=async()=>{};`}));
 await page.emulateMedia({reducedMotion:'reduce'});await page.goto('/?entrance=photo');await page.getByRole('button',{name:/(노트북 열고|휴대폰 들고) 작업실 들어가기/}).click();await expect(page.locator('#entrance')).toBeHidden();return writes;
}
test('guestbook publishes escaped text with server timestamp and owner-only deletion',async({page})=>{
 ...
 expect(writes[0].writes).toHaveLength(2);expect(writes[0].writes[0].update.fields.authorId.stringValue).toBe('test-guest');expect(writes[0].writes[1].update.name).toContain('/comment-cooldowns/test-guest');expect(writes[0].writes[0].update.fields.project.stringValue).toBe('guestbook');expect(writes[0].writes[0].updateTransforms).toEqual([{fieldPath:'createdAt',setToServerValue:'REQUEST_TIME'}]);
...
test('guestbook failed save preserves input and contact uses existing public values',async({page})=>{
 await prepare(page,{failure:true});await page.getByRole('button',{name:'방명록',exact:true}).click();await page.getByLabel('이름 또는 별명').fill('방문자');await page.getByLabel('남길 이야기').fill('다시 보낼 글');await page.getByRole('button',{name:'글 남기기',exact:true}).click();await expect(page.locator('#guestbook-status')).toContainText('1분에 한 번');await expect(page.getByLabel('남길 이야기')).toHaveValue('다시 보낼 글');await expect(page.getByRole('button',{name:'글 남기기',exact:true})).toBeEnabled();
 await page.getByRole('button',{name:'창 닫기',exact:true}).click();await page.getByRole('button',{name:'연락하기',exact:true}).click();await expect(page.getByRole('link',{name:'메일 쓰기',exact:true})).toHaveAttribute('href',/^mailto:/);await expect(page.getByRole('button',{name:'ID 복사',exact:true})).toBeVisible();await expect(page.getByRole('link',{name:'전화하기',exact:true})).toHaveCount(0);await page.screenshot({path:'.omo/evidence/studio/contact.png'});await page.setViewportSize({width:375,height:812});await expect(page.locator('#window-body')).toBeVisible();expect(await page.locator('#window-body').evaluate(el=>el.scrollWidth<=el.clientWidth+1)).toBe(true);await page.screenshot({path:'.omo/evidence/studio/contact-mobile.png'});
});
