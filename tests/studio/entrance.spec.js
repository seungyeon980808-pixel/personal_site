...
test('slow image loading preserves keyboard entry and cancellation',async({page})=>{
 ...
 try{
  await page.goto('/?entrance=closed',{waitUntil:'domcontentloaded'});await page.keyboard.press('Tab');await expect(page.locator('#enter')).toBeFocused();
...
test('approved photo endpoints and copy form one vertically centered hero',async({page})=>{
 for(const [width,height] of [[375,812],[1280,720],[1280,1000]]){
  await page.setViewportSize({width,height});await page.goto('/?entrance=closed');await page.locator('#entrance[data-ready="true"]').waitFor();
...
test('entry can be cancelled and retried, resize settles flight, exit restores keyboard focus',async({page})=>{
 await page.goto('/?entrance=closed');await expect(page.getByRole('heading',{level:1})).toHaveText('교사가 한가해야,교육이 성장한다.');
