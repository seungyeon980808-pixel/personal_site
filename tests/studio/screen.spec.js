...
for(const width of [375,1280])test(`screen content keeps identity and widget positions through entry at ${width}`,async({page})=>{
 await page.setViewportSize({width,height:812});await page.goto('/?entrance=closed');await page.locator('#entrance[data-ready="true"]').waitFor();
 await page.locator('#desktop').evaluate(node=>node.dataset.continuity='same-desktop');await expect(page.locator('#desktop')).toHaveAttribute('inert','');await expect(page.locator('.hammer-tool')).toBeDisabled();
 ...
 const before=await page.evaluate(()=>{const v=document.querySelector('#desktop').getBoundingClientRect();return {background:getComputedStyle(document.querySelector('#desktop'),'::before').backgroundImage,rects:['#widgets','#icons','#dock'].map(s=>{const r=document.querySelector(s).getBoundingClientRect();return [(r.x-v.x)/v.width,(r.y-v.y)/v.height,r.width/v.width,r.height/v.height];})};});
 await page.locator('#entrance').waitFor({state:'hidden'});
 const after=await page.evaluate(()=>{const v=document.querySelector('#desktop').getBoundingClientRect();return {background:getComputedStyle(document.querySelector('#desktop'),'::before').backgroundImage,rects:['#widgets','#icons','#dock'].map(s=>{const r=document.querySelector(s).getBoundingClientRect();return [(r.x-v.x)/v.width,(r.y-v.y)/v.height,r.width/v.width,r.height/v.height];})};});
 ...
 await expect(page.locator('.hammer-tool')).toBeEnabled();
...
test('hover peeks without entering, pointer leave closes and click continues',async({page})=>{
 await page.goto('/?entrance=closed');await page.locator('#entrance[data-ready="true"]').waitFor();await page.mouse.move(1,1);
 const points=()=>page.locator('.device-cover').evaluate(e=>{const s=getComputedStyle(e),m=new DOMMatrix(s.transform),o=s.transformOrigin.split(' ').map(parseFloat);return [747/1024,851/1024].map(y=>{const p=new DOMPoint(0,e.clientHeight*y-o[1]).matrixTransform(m);return p.y+o[1];});});
