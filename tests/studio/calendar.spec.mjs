...
async function prepare(page,{owner=false}={}){
 ...
 await page.emulateMedia({reducedMotion:'reduce'});await page.goto('/');const enter=page.getByRole('button',{name:/(노트북 열고|휴대폰 들고) 작업실 들어가기/,exact:true});if(await enter.isVisible())await enter.click();await expect(page.locator('#entrance')).toBeHidden();
}
...
test('owner drag-snaps a schedule and record CRUD persists in the private draft',async({page})=>{
 ...
 await expect(page.getByRole('heading',{name:'추가하기'})).toBeVisible();const start=await page.getByLabel('시작').inputValue(),end=await page.getByLabel('종료').inputValue();expect(Number(start.split(':')[1])%10).toBe(0);expect(Number(end.split(':')[1])%10).toBe(0);
