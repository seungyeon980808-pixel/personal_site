...
export function applyIconVariant(style) {
 const selected = ['ios', 'glass', 'mono', 'object', 'color'].includes(style) ? style : 'ios';
 document.documentElement.dataset.iconStyle = selected;
 document.querySelectorAll('.icon[data-symbol]').forEach(icon => {
  const name = icon.dataset.symbol;
  if (!Object.hasOwn(glyphs, name)) return;
  icon.style.setProperty('--variant-top', colors[name][0]);
  icon.style.setProperty('--variant-bottom', colors[name][1]);
  icon.innerHTML = renderIcon(name, selected);
 });
 return selected;
}
