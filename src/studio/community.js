...
const documentRoot='projects/edunote-96bd7/databases/(default)/documents/comments/';
...
export function guestbook(){
 ...
 async function refresh(){
  ...
  try{
   const rows=await request(root+':runQuery',json({structuredQuery:{from:[{collectionId:'comments'}],where:{fieldFilter:{field:{fieldPath:'project'},op:'EQUAL',value:{stringValue:'guestbook'}}}}}));
   if(!list.isConnected)return;
   const entries=rows.filter(row=>row.document).map(({document:d})=>({id:d.name.split('/').at(-1),name:d.fields.name?.stringValue||'',text:d.fields.text?.stringValue||'',date:d.fields.createdAt?.timestampValue||''})).sort((a,b)=>b.date.localeCompare(a.date));
 ...
 form.onsubmit=async event=>{
  ...
  try{
   await request(root+':commit',json({writes:[{update:{name:documentRoot+crypto.randomUUID(),fields:{project:{stringValue:'guestbook'},name:{stringValue:name},text:{stringValue:text}}},updateTransforms:[{fieldPath:'createdAt',setToServerValue:'REQUEST_TIME'}],currentDocument:{exists:false}}]}));
...
export function contact(){
 const fields=state.site.fields,email=plain(fields['contact.email']).trim(),kakao=plain(fields['contact.kakao']).trim(),phone=plain(fields['contact.phone']||'010-4917-4332').trim(),chat=safeURL(plain(fields['contact.openchat']));
