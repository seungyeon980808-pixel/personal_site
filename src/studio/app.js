...
function render(){
 ...
 $('#dock').innerHTML=`<button class="dock-launcher" data-route='{"view":"programs"}'>${icon('folder')}<span>만든 도구</span></button>`+state.site.programs.slice(0,5).map(p=>`<button data-route='${JSON.stringify({view:'program',id:p.id})}' aria-label="${e(p.label)}">${icon('sheet',p.icon)}<span>${e(p.label)}</span></button>`).join('')+`<button data-route='{"view":"records"}'>${icon('calendar')}<span>캘린더</span></button>`;
 calendar();
}
function calendar(){
...
configureWindow(route);initWindow();initEntrance();
