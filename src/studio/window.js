...
export function close(){document.dispatchEvent(new Event('studio:windowclose'));$('#workspace-window').close();$('#window-body').replaceChildren();$('#restore').hidden=true;trail=[];focusBefore?.focus();}
...
export function content(title,html){$('#workspace-window').classList.remove('browser-window');document.dispatchEvent(new Event('studio:windowclose'));$('#window-title').textContent=title;$('#window-body').innerHTML=html;$('#window-body').scrollTop=0;}
