...
try{
 for(const [width,height] of [[375,812],[768,900],[1280,900],[1280,720]]){
  ...
  const shot=async name=>{const file=`${name}-${width}x${height}.png`;await page.screenshot({path:`${dir}/${file}`});record.files.push(file);};
  ...
  await page.locator('#enter').click();await page.waitForTimeout(420);await shot('opening');await page.locator('#entrance[data-phase="open"]').waitFor();await shot('open');await page.locator('#entrance[data-phase="zooming"]').waitFor();await page.waitForTimeout(300);await shot('zoom');
  await page.locator('#entrance').waitFor({state:'hidden'});await shot('desktop');await axe('desktop');
  for(const [label,name] of [['추천 도구함','finder'],['만든 도구','programs'],['메모 펼쳐보기 ↗','note'],['캘린더','records']]){
  ...
  await page.getByRole('button',{name:'5E',exact:true}).click();await shot('chooser');await page.getByRole('button',{name:/바로 실행/}).click();await shot('browser');
  ...
  await page.getByRole('button',{name:'노트북 열고 작업실 들어가기'}).click();await page.keyboard.press('Escape');await page.locator('#entrance[data-phase="closed"]').waitFor();await shot('cancelled');
