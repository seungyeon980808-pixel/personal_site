...
try{for(const width of [375,1280]){
 ...
 await audit('desktop');for(const name of ['만든 도구','추천 도구함','연수 자료','공유 자료','소개','캘린더']){await page.getByRole('button',{name,exact:true}).first().click();await audit(name);await page.locator('#window-close').click();}
